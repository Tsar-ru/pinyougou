...
app.controller('goodsController' ,function($scope,$controller ,itemCatService  ,goodsService){	
    ...
    $scope.marketStatus=['未上架','已上架','已下架'];  //0    1    2

	$scope.updateIsMarketable=function(status,info){
        ...
        if(flag){
            goodsService.updateIsMarketable(status,$scope.selectIds).success(function (response) {
                if(response.success){
                    $scope.reloadList();
                    $scope.selectIds=[];
                }else{
	...
	$scope.findAllItemCat=function(){
        itemCatService.findAll().success(function (response) {
           /* response:[{"id":1,"name":"图书、音像、电子书刊","parentId":0,"typeId":35},
				{"id":559,"name":"手机通讯","parentId":558,"typeId":35},
				{"id":560,"name":"手机","parentId":559,"typeId":35}  ]
			 -------> itemCat：{1："图书、音像、电子书刊"，559："手机通讯"，560："手机"}*/
